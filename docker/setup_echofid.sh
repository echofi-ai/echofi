...
GENESIS_FILE="$HOME"/.echofi/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  echo "$GENESIS_FILE exists..."
else
  echo "$GENESIS_FILE does not exist. Generating..."

  echofid init --chain-id "$CHAIN_ID" "$MONIKER"
  # staking/governance token is hardcoded in config, change this
  sed -i "s/\"uecho\"/\"$STAKE\"/" "$GENESIS_FILE"
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.echofi/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.echofi/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.echofi/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.echofi/config/config.toml
...
if ! echofid keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | echofid keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | echofid genesis add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    echofid genesis add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE,5000000000uusd"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | echofid genesis gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
  # (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | echofid genesis gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID"
